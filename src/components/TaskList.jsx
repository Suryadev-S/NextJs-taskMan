...
const TaskList = ({ activeBoardId }) => {
  ...
  const activeBoardTasks = tasks.filter(
  ...
  const handleSubTaskUpdate = (taskId, subIndex, isDone) => {
    const updatedTasks = tasks.map((task) => {
      if (task.taskId == taskId) {
        ...
        const subsDoneCount = updatedSubs.reduce(
                ...
                <CardHeader>
                  <Dialog>
                    <DialogTrigger asChild>
                      <CardTitle
                        id={`${task.isDone ? "cardTitleDone" : "cardTitle"}`}
                      >
                        {task.title}
                      </CardTitle>
                    </DialogTrigger>
                        ...
                        <DialogTitle>
                          ...
                          {task.subTasksCount == 0 && (
                            <Checkbox
                              checked={task.isDone}
                              onCheckedChange={(e) => {
                                handleUpdateWholeTask(task.taskId, e);
                              }}
                            />
                          )}
                        </DialogTitle>
                              ...
                              return (
                                <li key={j}>
                                  <Checkbox
                                    ...
                                    checked={sub.isDone}
                                    onCheckedChange={(e) =>
                                      handleSubTaskUpdate(task.taskId, j, e)
                                    }
                                  />
                                  &nbsp;
                                  <label htmlFor="subTask">{sub.subTask}</label>
                                </li>
                              );
                            })}
                          </ul>
                  ...
                  <CardDescription id="cardDesc">
                    {task.subTasksDone} of {task.subTasksCount}
                  </CardDescription>
                </CardHeader>
                <CardFooter>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      ...
                      <DropdownMenuItem asChild>
                        <TaskDialog taskToEdit={task} activeBoardId={activeBoardId}/>
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </CardFooter>
